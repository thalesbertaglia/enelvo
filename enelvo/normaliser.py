...
class Normaliser:
    ...
    def __init__(self, main_lex, es_lex, pn_lex, ac_lex, fc_list, ig_list, ok_lex, tokenizer, norm_lex, threshold, n_cands, capitalize_inis, capitalize_pns, capitalize_acs, sanitize, logger):
        ...
        self.ok_lex = {**main_lex, **es_lex, **pn_lex, **ac_lex}
        # Lexicon of internet slang
        self.in_lex = loaders.load_lex_corr(file_path=in_lex)
        self.ok_lex = {k: ok_lex[k] for k in ok_lex if k not in in_lex}
        self.ok_lex = {**ok_lex, **ig_list} if ig_list else ok_lex
